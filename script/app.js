...
const showResult = (queryResponse) => {
	...
	const timeDifference = (queryResponse.city.sunset - queryResponse.city.sunrise) / 60; //API = seconden => /60 voor seconden
...
const getAPI = async (lat, lon) => {
	...
	const data = await fetch(`http://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${KEY}&units=metric&lang=nl&
	cnt=1`)
		.then((r) => r.json())
